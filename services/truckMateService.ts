...
export const createOrder = async (clientData) => {
  ...
  const requestBody = {
    ...
    aCharges: clientData.aCharges.map(({ aChargeCode, quantity, rate }) => ({
      aChargeCode,
      quantity: parseInt(quantity, 10),
      rate: parseFloat(rate),
    })),
    startZone: pickup.postalCode,
    endZone: dropoff.postalCode,
  }
...
/*
 * Retrieves user field records for the specified source type.
 * https://developer.trimble.com/docs/truckmate/truckmate-api#get-/userFieldsData
 *
 * @param {string} sourceType - Type of user fields. Allowed values are
 * 'orderInterliner' (default) and 'tripCarrier'
 */
export const getUserFields = async (sourceType = 'orderInterliner') => {
  const url = `${TRUCKMATE_API_URL}/userFieldsData`
  let response
  ...
  try {
    ...
    response = data
    console.log('User fields data response:', response)
  } catch (error) {
    let errorMessage = 'Failed to fetch user fields: '

    if (error instanceof Error) {
      console.error('User fields fetch error details:', {
        message: error.message,
        stack: error.stack,
        url,
      })
