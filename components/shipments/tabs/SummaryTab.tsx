...
export function SummaryTab({ formData }: SummaryTabProps) {
  const totalWeight =
    formData.commodities?.reduce(
      (sum: number, item) => sum + Number(item.weight || 0),
      0
    ) || 0;
  const totalPieces =
    formData.commodities?.reduce(
      (sum: number, item) => sum + Number(item.pieces || 0),
      0
    ) || 0;
  const totalPallets =
    formData.commodities?.reduce(
      (sum: number, item) => sum + Number(item.pallets || 0),
      0
    ) || 0;

  const totalCustomerRate =
    formData.customerCharges?.reduce(
      (sum: number, charge) => sum + Number(charge.amount || 0),
      0
    ) || 0;
  const totalCarrierRate =
    formData.carrierCharges?.reduce(
      (sum: number, charge) => sum + Number(charge.amount || 0),
      0
    ) || 0;
    ...
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-3 gap-8'>
        <div>
          ...
          <div className='space-y-1'>
            ...
            <p className='text-gray-600'>
              Revenue: ${totalCustomerRate.toLocaleString()}
            </p>
        ...
        <div className='text-right'>
          <p className='text-gray-700 mb-2'>
            Total Customer Rate: ${totalCustomerRate.toLocaleString()}
          </p>
          <p className='text-gray-700'>
            Total Carrier Rate: ${totalCarrierRate.toLocaleString()}
          </p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className='flex justify-between items-center text-gray-600 mb-2'>
          <span>
            Total: {totalWeight} lb, {totalPieces} pieces, {totalPallets}{" "}
            Pallets
          </span>
        </div>
        {formData.commodities?.map((commodity, index) => (
          <div
            key={index}
            className='flex justify-between py-2 border-t border-gray-200'
          >
            <span>Commodity Description [Code #{index + 1}]</span>
            <span className='text-gray-600'>
              {commodity.weight} lbs, {commodity.pieces} pieces,{" "}
              {commodity.pallets} pallets
            </span>
          </div>
        ))}
      </div>
